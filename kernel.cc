...
void* kalloc(size_t sz) {
    ...
    if (next_alloc_pa == 0) {
        ...
        check_var = next_alloc_pa - PAGESIZE;
    }

    while (next_alloc_pa != check_var) {
...
void kfree(void* kptr) {
    ...
    if (kptr != nullptr) {
        if (pages [(uintptr_t) kptr/PAGESIZE].refcount > 1) {
...
void process_setup(pid_t pid, const char* program_name) {
    ...
    memset (ptable[pid].pagetable, 0x0, PAGESIZE);
    ...
    for (loader.reset(); loader.present(); ++loader) {
        for (uintptr_t a = round_down(loader.va(), PAGESIZE);
            ...
            uintptr_t temp_ptr = (uintptr_t) kalloc(PAGESIZE);
            if (!loader.writable()) {
                vmiter (ptable[pid].pagetable, a).map(temp_ptr, PTE_P | PTE_U) ;
            } else {
                vmiter(ptable[pid].pagetable, a).map(temp_ptr, PTE_P | PTE_W | PTE_U) ;
            }
        }
    }
    ...
    for (loader.reset(); loader.present(); ++loader) {
        vmiter iter = vmiter(ptable[pid].pagetable, loader.va());
    ...
    uintptr_t stack_ptr = (uintptr_t) kalloc(PAGESIZE);
    uintptr_t stack_addr = MEMSIZE_VIRTUAL - PAGESIZE;
    vmiter(ptable[pid].pagetable, stack_addr).map(stack_ptr, PTE_P | PTE_U | PTE_W);
...
int syscall_page_alloc(uintptr_t addr) {
    ...
    if ((addr < PROC_START_ADDR && addr != CONSOLE_ADDR) || addr > MEMSIZE_VIRTUAL) {
        return -1;
    }
    uintptr_t kptr = (uintptr_t) kalloc(PAGESIZE);
    if ((void*) kptr == nullptr){
        return -1;
    }
    vmiter(current->pagetable, addr).map(kptr, PTE_P | PTE_W | PTE_U);
    return 0;
}

// Small helper fn to free pagetables used in syscall_fork
// also used in syscall_exit

void page_free(x86_64_pagetable* page_table) {
    for (vmiter it(page_table, 0); it.va() < MEMSIZE_VIRTUAL; it += PAGESIZE) {
        if (it.user() && it.va() != CONSOLE_ADDR) {
            kfree((void*) it.pa()) ;
        }
    }
    for (ptiter it(page_table) ; it.active() ; it.next ()) {
        kfree(it.kptr()) ;
    }
    kfree(page_table);
    }
...
pid_t syscall_fork() {
    ...
    for (int a = 1; a < NPROC; a++){
        if (ptable[a].state == P_FREE ){
            ptable[a].pagetable = (x86_64_pagetable*) kalloc(PAGESIZE);
            if (ptable[a].pagetable == nullptr) {
                return -1;
            }
            memset (ptable[a].pagetable, 0x0, PAGESIZE);
            for (vmiter it(current->pagetable, 0); it.va() < MEMSIZE_VIRTUAL; it += PAGESIZE) {
                int var;
                if (!it.user() || it.va() == CONSOLE_ADDR) {
                    var = vmiter(ptable[a].pagetable, it.va()).try_map(it.pa(), it.perm()) ;
                } else if (it.writable ()) {
                    void* curr_page = kalloc(PAGESIZE) ;
                    if (curr_page == nullptr) {
                        page_free(ptable[a].pagetable);
                        return -1;
                    }
                    memcpy(curr_page, (void*) it.pa(), PAGESIZE) ;
                    var = vmiter(ptable[a].pagetable, it.va()).try_map(curr_page, it.perm()) ;
                    if (var == -1) {
                        kfree (curr_page);
                    }
                } else {
                    var = vmiter(ptable[a].pagetable, it.va()).try_map(it.pa() , PTE_P | PTE_U) ;
                    if (var == 0) {
            ...
            ptable[a].pid = a;
            ptable[a].regs = current->regs;
            ptable[a].regs.reg_rax = 0;
            ptable[a].state = P_RUNNABLE;
            return a;
        }
    }
    return -1;
